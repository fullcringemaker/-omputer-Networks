...
func handleSendMessagePost(w http.ResponseWriter, r *http.Request) {
	...
	// Если отправитель - это текущий пир, то также отображаем сообщение на веб-сайте
	if req.Sender == peerName {
		broadcastMessage(fmt.Sprintf("Sent message to %s: %s", req.Recipient, req.Message))
	}
...
func handleSendMessageGet(w http.ResponseWriter, r *http.Request) {
	...
	// Если отправитель - это текущий пир, то также отображаем сообщение на веб-сайте
	if sender == peerName {
		broadcastMessage(fmt.Sprintf("Sent message to %s: %s", recipient, message))
	}
